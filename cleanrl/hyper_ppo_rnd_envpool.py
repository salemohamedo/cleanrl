...
def parse_args():
    ...
    parser.add_argument("--wandb-entity", type=str, default='mila-projects',
    ...
    parser.add_argument("--name", type=str, default="hyper_rnd_ppo")

    ## Hyperbolic args
    parser.add_argument('--effective_softmax_rescale', type=float, default=0.5) 
    parser.add_argument('--rescale_normal_params', type=lambda x: bool(strtobool(x)), default=True)
    parser.add_argument('--rescale_euclidean_norms_gain', type=float, default=1.0)
    parser.add_argument('--dimensions_per_space', default=32)
    parser.add_argument('--spectral_norm', type=lambda x: bool(strtobool(x)), default=True)
    parser.add_argument('--clip_norm', type=lambda x: bool(strtobool(x)), default=True)
    parser.add_argument('--max_norm', type=int, default=60)
    parser.add_argument('--final-feature-layer-dim', type=int, default=512) 
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            layer_init(nn.Linear(64 * 7 * 7, args.final_feature_layer_dim)),
            ClipNorm(max_norm=args.max_norm, dimensions_per_space=32) if args.clip_norm else nn.Identity(),
        )
        ...
        hyper_kw_args = {
            ...
            "rescale_normal_params": args.rescale_normal_params,
            "rescale_euclidean_norms_gain" : args.rescale_euclidean_norms_gain,
            "dimensions_per_space" : args.dimensions_per_space,
        }
        self.actor = PoincarePlaneDistance(args.final_feature_layer_dim, envs.single_action_space.n, **hyper_kw_args)
        self.critic_ext = PoincarePlaneDistance(args.final_feature_layer_dim, 1, **hyper_kw_args)
        self.critic_int = PoincarePlaneDistance(args.final_feature_layer_dim, 1, **hyper_kw_args)
    ...
    def get_hyperbolic_rep_magnitude(self, obs):
        features = self.network(obs / 255.0)
        hyp_features = self.actor.map_to_ball(features)
        origin_dist = self.actor.ball.dist0(hyp_features) 
        return origin_dist
    
    def compute_intrinsic_reward(self, obs):
        origin_dist = self.get_hyperbolic_rep_magnitude(obs)
        ## Closer to ball origin == more uncertainty
        return -origin_dist
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}__{str(date.today())}"
    if args.track:
        import wandb
    ...
    optimizer = geoopt.optim.RiemannianAdam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            ## Log hyperbolic magnitude
            hyper_magnitude = agent.get_hyperbolic_rep_magnitude(next_obs).detach().cpu().numpy()
            writer.add_scalar("charts/avg_hyperbolic_magnitude_mean", hyper_magnitude.mean(), global_step)
            writer.add_scalar("charts/avg_hyperbolic_magnitude_std", hyper_magnitude.std(), global_step)

            curiosity_rewards[step] = agent.compute_intrinsic_reward(next_obs).data
